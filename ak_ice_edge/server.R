...
Mos <- reactive({
	c(paste0("0", 1:9), 10:12)[Months_Int()]
})

Mos_Abb <- reactive({
	month.abb[Months_Int()]
})

ncols <- reactive({
	if(input$color_by=="Decade") ncols <- length(Mos()) else ncols <- length(Decades())
	ncols
})

nrows <- reactive({
	if(input$color_by=="Decade") nrows <- length(Decades()) else nrows <- length(Mos())
	nrows
})
...
Main_title <- reactive({
	if(input$include_annual) "Annual and Mean Decadal 15% Sea Ice Concentration Edges" else "Mean Decadal 15% Sea Ice Concentration Edge"
})
...
Key_text <- reactive({
	if(input$color_by=="Decade"){ # Designed for up to two columns in the plot key
		keytext <- list(rep(Decades(),ncols()))
	} else if(input$color_by=="Month") {
		keytext <- list(rep(Mos_Abb(),ncols()))
	}
	keytext
})

#clrs <- rev(brewer.pal(11, "Spectral")[c(1:3, 9:11)]) # Number of colors must equal number of elements in 'input$color_by' variable
...
doPlot_plotMap <- function(...){
	if(!(is.na(Mos()[1]) | !length(Decades()) | is.null(nrows()) | is.null(ncols()) | is.null(Key_text()) | is.null(Key_title()) | is.null(SLDF_names()) | is.null(input$include_annual))){
		plotMap(r=r3, nr=nrows(), nc=ncols(), key.text=Key_text(), key.title=Key_title(), sldf.names=SLDF_names(), sldf.annual.names=SLDF_annual_names(), annual=input$include_annual, main.title=Main_title(), ...)
	} else NULL
}

# Currently allowing download as PNG only
plotMap <- function(r, nr, nc, key.text, key.title, sldf.names, sldf.annual.names, annual=FALSE, main.title="Plot", PDF=FALSE, PNG=FALSE, filename=NULL, show.logo=F, logo.mat=NULL, verbose=TRUE){
	if(verbose){
	...
	p <- levelplot(r, maxpixels = ncell(r/10),
		...
		par.settings=modifyList(theme1, list( superpose.symbol=list(col=clrs[1:nr], lty=1, lwd=1, fill="white"), layout.heights=list(top.padding=pad, bottom.padding=pad/1.25) )),
		key=list(
			x=1-0.05*(nrow(r)/ncol(r)), y=0.05, corner=c(1, 0), size=3,
			lines=list(lty=rep(c(1,2),each=nr)[1:(nr*nc)], lwd=2, col=clrs[1:nr]),
			text=key.text,
			fontfamily=font.fam,
	...
	if(annual){
		for(i in 1:nr){ # Add annual lines
			...
			#Sys.sleep(0.2)
			p <- p + layer(sp.lines(x, col=clrs[i], lwd=1, alpha=0.5), data=list(x=get(sldf.annual.names[i]), i=i))
			if(nc>1) p <- p + layer(sp.lines(x, col=clrs[i], lwd=1, lty=5, alpha=0.5), data=list(x=get(sldf.annual.names[i+nr]), i=i))
		}
	}
	...
	for(i in 1:nr){ # Add decadal lines
		...
		#Sys.sleep(0.2)
		p <- p + layer(sp.lines(x, col=clrs[i], lwd=2), data=list(x=get(sldf.names[i]), i=i))
		if(nc>1) p <- p + layer(sp.lines(x, col=clrs[i], lwd=2, lty=5), data=list(x=get(sldf.names[i+nr]), i=i))
	}
	# Overlay "background"
	for(i in 1:3){
