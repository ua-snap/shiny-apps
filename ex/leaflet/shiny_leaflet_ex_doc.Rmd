...
files <- normalizePath(list.files(pattern="ex_0.*.shiny.R", full.names=TRUE))
opts_knit$set(root.dir = dirname(files[1]))
lapply(files, read_chunk)
setwd(dirname(files[1]))
library(shiny)
knit_hooks$set(set.dir = function(before, options, envir) {
    if (before) setwd("/var/www/shiny-server/shiny-apps/ex/leaflet")
})
```

```{r, super_chunk, set.dir=TRUE}
...
```{r, echo=FALSE}
shinyAppFile(files[1], options=list(width="100%", height=400, root.dir=dirname(files[1])))
```
...
```{r, echo=FALSE}
shinyAppFile(files[2], options=list(width="100%", height=400, root.dir=dirname(files[1])))
```
