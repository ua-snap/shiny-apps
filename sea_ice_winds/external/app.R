...
output$plotByYear <- renderPlot({ # render plot for mainPanel tabsetPanel tabPanel
		if(!is.null(w.prop.dec()) & !is.null(i.prop.dec()) & !is.null(input$annstyle)){
			tsPlot(w.prop.yrs()$Year,w.prop.yrs()$Freq,i.prop.yrs()$Con,input$yrs,v1name=paste(input$mod,input$var),v2name="Sea ice",style=input$annstyle,cex1=cex,
...
output$dl_plotByYear <- downloadHandler( # render plot to pdf for download
	...
	content = function(file){
		...
		tsPlot(w.prop.yrs()$Year,w.prop.yrs()$Freq,i.prop.yrs()$Con,input$yrs,v1name=paste(input$mod,input$var),v2name="Sea ice",style=input$annstyle,cex1=cex-0.4,
...
output$plotByDecade <- renderPlot({ # render plot for mainPanel tabsetPanel tabPanel
		if(!is.null(w.prop.dec()) & !is.null(i.prop.dec()) & !is.null(input$decstyle)){
			if(nrow(i.prop.dec())>1){
				tsPlot(w.prop.dec()$Year,w.prop.dec()$Freq,i.prop.dec()$Con,decadal=T,input$yrs,v1name=paste(input$mod,input$var),v2name="Sea ice",style=input$decstyle,xaxt="n",cex1=cex,
...
output$dl_plotByDecade <- downloadHandler( # render plot to pdf for download
	...
	content = function(file){
		pdf(file = file, width=12, height=4)
		tsPlot(w.prop.dec()$Year,w.prop.dec()$Freq,i.prop.dec()$Con,decadal=T,input$yrs,v1name=paste(input$mod,input$var),v2name="Sea ice",style=input$decstyle,xaxt="n",cex1=cex-0.4,
