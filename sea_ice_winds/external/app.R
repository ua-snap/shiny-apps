...
output$plotByYear <- renderPlot({ # render plot for mainPanel tabsetPanel tabPanel
		if(!is.null(w.prop.dec()) & !is.null(i.prop.dec())){
			tsPlot(w.prop.yrs()$Year,w.prop.yrs()$Freq,i.prop.yrs()$Con,yrs(),v1name=paste(input$mod,input$rcp,input$var),v2name="Composite RCP 8.5 Sea ice", cex1=cex,
...
output$dl_plotByYear <- downloadHandler( # render plot to pdf for download
	...
	content = function(file){
		...
		tsPlot(w.prop.yrs()$Year,w.prop.yrs()$Freq,i.prop.yrs()$Con,yrs(),v1name=paste(input$mod,input$rcp,input$var),v2name="Composite RCP 8.5 Sea ice", cex1=cex-0.4,
			xlb="Year",ylb=ylab.ann,mn=main.ann())
		dev.off()
	}
)
...
output$plotByDecade <- renderPlot({ # render plot for mainPanel tabsetPanel tabPanel
		if(!is.null(w.prop.dec()) & !is.null(i.prop.dec())){
			if(nrow(i.prop.dec())>1){
				tsPlot(w.prop.dec()$Year,w.prop.dec()$Freq,i.prop.dec()$Con,decadal=T,yrs(),v1name=paste(input$mod,input$rcp,input$var),v2name="Composite RCP 8.5 Sea ice", xaxt="n",cex1=cex,
...
output$dl_plotByDecade <- downloadHandler( # render plot to pdf for download
	...
	content = function(file){
		pdf(file = file, width=12, height=4)
		tsPlot(w.prop.dec()$Year,w.prop.dec()$Freq,i.prop.dec()$Con,decadal=T,yrs(),v1name=paste(input$mod,input$rcp,input$var),v2name="Composite RCP 8.5 Sea ice", xaxt="n",cex1=cex-0.4,
