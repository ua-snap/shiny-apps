function(){
	tabPanel("About",
		...
		Individual months can be highlighted by differently colored time series lines.</p>
		...
		The user can select any one of these variables as a conditional variable to generate a comparative visual display of extreme events bar plots by factor levels of the chosen variable.</p>
		
		<p style="text-align:justify"><strong>Notes:</strong> Geographic locations are represented by the names of the largest population centers,
		...
		Values for all GCMs are at a common 2.5 x 2.5 degree resolution after having been regridded for quantile mapping to ERA-40. Therefore the scale is much larger than the place names might suggest.</p>
		...
		To clarify, the term, above thresholds, for wind velocity components means greater than the selected threshold value(s), as it does for wind speed and temperature. It does not mean further from zero.</p>
		
		<p style="text-align:justify">Note that any time the Wind variable is selected, wind threshold options will be limited.
		...
		As such, there are no plans for continued development or refinement of this particular app.</p>
		
		<p style="text-align:justify"><strong>Suggestions:</strong> You may download a graphic in pdf form for your convenience using the download button.
		...
		Formatting of the pdf plot will not match the browser plot exactly, but it will be a close approximation.</p>
		...
		This should not be an issue since more than approximately four locations compared at one time will be too visually cluttered anyhow.</p>
		...
		<br/>
		<a href="http://www.snap.uaf.edu/data.php#dataset=Historical_Daily_Mean_Quantile_Mapped_Temperatures" target="_blank">Quantile-mapped historical daily temperature</a>
		<br/>
		<a href="http://www.snap.uaf.edu/data.php#dataset=Projected_Daily_Mean_Quantile_Mapped_Temperatures" target="_blank">Quantile-mapped projected daily temperature</a>
		<br/>
