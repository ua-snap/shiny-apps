...
All of these serve in subsetting the two main continuous numerical variables (temperature and precipitation) by way of selecting factor levels and ranges, respectively.
Going row by row, first are the climate variables, temperature and precipitation. To the right, units of measurement, metric selected by default.
...
Months and decades follow. Finally, in the last row of inputs the user can choose a spatial scale on the left.
On the right, geographic locations propagate.
...
Because months and decades are factors with a number of levels, and often all levels are selected, it is tedious to have to check (or remove) them all.
It also wastes space. For these two menus it is assumed the user wants all levels *unless* they specify a subset.
The years of data actually returned by the app are represented by the intersection of the years implied by the decades selection and the years implied by the years slider.
...
seasonal and multi-decadal specifications each require that the user explicitly select months and decades, respectively.
If only one month or one decade are explicitly selected, seasonal and multi-decadal aggregation remain unavailable because there is nothing to aggregate yet.

If the user explicitly selects two or more months, a checkbox appears for making equal-length seasons from the selected months.
...
For example, if six months are selected, the options include aggregating to three 2-month seasons, two 3-month seasons, or one 6-month season.
...
If the months or decades selected are not continuous (e.g., January, February, and July, or 1950s, 1960s, 1990s, and 2000s), each continuous subset is checked sequentially.
...
For equal-length subsets, the number of equal-length seasons available to choose from is restricted to the maximum possible for the individual subsets, not the entire selection.
