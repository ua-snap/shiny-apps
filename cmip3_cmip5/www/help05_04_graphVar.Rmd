...
The `Show CRU 3.1` checkbox includes CRU among GCMs already selected rather than a stricty overlay as in the time series plot.
...
As a result, menus within the options panel change dynamically based upon certain selections, more so than for other plots.
When bars are shown, a checkbox is available to center the data around the mean.
Checking this box changes the plot to a line plot of the original data with different options for error bars around the mean line at each value along the x-axis.
The error bars are the same as what is otherwise shown in terms of magnitude of variation when using just a bar plot.
This all goes away is the boxplot option is checked, providing a completely different plot type and associated options.
