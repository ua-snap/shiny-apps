function(d, d.grp, d.pool, x, y, stat="SD", around.mean=FALSE, error.bars=FALSE, panels, grp, n.grp, ingroup.subjects=NULL, facet.cols=min(ceiling(sqrt(panels)),5), facet.by, vert.facet=FALSE, fontsize=16,
	colpal, colseq, altplot, boxplots=FALSE, pts.alpha=0.5, bartype, bardirection, show.points=FALSE, show.overlay=FALSE, overlay=NULL, jit=FALSE, plot.title="", plot.subtitle="", lgd.pos="Top", units=c("C","mm"),
	mos=12, yrange, clbootbar, clbootsmooth, show.logo=F, logo.mat=NULL){
		...
		if(!is.null(d)){
			...
			if(around.mean) {
				...
				if(!is.factor(d[[x]])) g <- g + annotate("text", y=max.val, x=min(d[[x]]), label=bquote(.(plot.subtitle)), hjust=0, vjust=1, fontface=3)
			}
			if(show.logo){
