output$tsText <- renderUI({
	HTML("<h3>Data Selection</h3>
		<ul>
			<li>Must select at least one climate variable, scenario, corresponding model, month, decade, and area (region or city) to plot.</li>
...
output$spText <- renderUI({
	HTML("<h3>Data Selection</h3>
		<ul>
			<li>Must select two climate variables, at least one scenario, corresponding model, month, decade, and area (region or city) to plot.</li>
...
output$hmText <- renderUI({
	HTML("<h3>Data Selection</h3>
		<ul>
			<li>Must select at least one climate variable, scenario, corresponding model, month, decade, and area (region or city) to plot.</li>
			<li>If multiple climate variables are selected, the data subset is truncated to the first in the list.
			The heat map is bivariate but X and Y are categorical variables. The data, Z, may be temperature or precipitation values but not both.</li>
			<li>If months or decades left blank, selection defaults to all. Select only what you want, but no need to click every factor level.</li>
			<li>Years are the intersection of the years slider and the decades list. One may suffice, unless you want partial or discontinuous decades.</li>
			<li>You may subset data when minimally required selections are complete. This text will vanish and a blue subset button will appear.</li>
			<li>You may download data when subset is complete. The data selection panel will minimize and the plot options panel will appear below.</li>
...
output$SubsetTableHeatmap <- renderDataTable({ if(!is.null(dat_heatmap())) dat()[-9] }, options=list(bSortClasses = TRUE, aLengthMenu=c(5, 10, 25, 50), iDisplayLength=5)) # same as table 1

output$TableHeatmap <- renderUI({
	if(is.null(input$goButton) || input$goButton==0) return()
	isolate(
		if(permitPlot()) fluidRow(column(12, dataTableOutput("SubsetTableHeatmap")))
	)
})
