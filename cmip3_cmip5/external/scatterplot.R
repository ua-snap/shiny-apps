function(d, form.string, panels, grp, n.grp, facet.cols=ceiling(sqrt(panels)), facet.by, vert.facet=FALSE, fontsize=16,
	colpal, colseq, contourlines=FALSE, hexbin=FALSE, pts.alpha=0.5, show.overlay=FALSE, overlay=NULL, jit=FALSE,
	...
	mos=12, pooled.var, show.logo=F, logo.mat=NULL){
		...
		if(!is.null(d)){
			...
			if(show.overlay){
				...
				g <- g + geom_point(data=overlay, aes_string(x=x, y=y, colour=NULL, fill=NULL), position=point.pos, pch=21, size=4, fill="black", colour="red", alpha=pts.alpha)
