function(d, x, y, panels, facet.cols=ceiling(sqrt(panels)), facet.by, fontsize=16, colpal, reverse.colors=FALSE, aspect_1to1=FALSE, show.values=FALSE,
	show.overlay=FALSE, overlay=NULL, plot.title="", plot.subtitle="", show.panel.text=FALSE, show.title=FALSE, lgd.pos="Top", units=c("C","mm"),
	pooled.var, show.logo=F, logo.mat=NULL){
		...
		g <- ggplot(d, aes_string(x=x, y=y, fill="Val")) + geom_tile(colour="black") + theme_bw(base_size=fontsize) + theme(legend.position=tolower(lgd.pos))
		...
		if(show.values) g <- g + geom_text(aes(fill=Val, label=round(Val, 1)))
