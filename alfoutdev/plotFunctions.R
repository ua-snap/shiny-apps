...
plotFRPbyBuffer <- function(data, min.buffer, colpal, subject, grp="", fontsize=16, leg.pos="top", maintitle="", xlb="", ylb="", facet.by=NULL, facet.cols=1){
...
plotFRIboxplot <- function(d, x, y, grp=NULL, Log=FALSE, colpal, show.points=TRUE, pts.alpha=1, fontsize=16, leg.pos="top", facet.by=NULL, facet.cols=1, lgd.pos="top"){
	d$Buffer_km <- factor(d$Buffer_km)
	if(Log) d$FRI <- log(d$FRI + 1)
	...
	#d <- subset(d, Buffer_km %in% as.numeric(buffersize) & Year >= year.range[1] & Year <= year.range[2])
	xlb=x
	#maintitle <- paste(year.range[1], "-", year.range[2], "Fire Return Interval Distributions")
