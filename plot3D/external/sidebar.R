	...
	conditionalPanel(condition="input.tsp!=='rcode'",
		wellPanel(
			...
			conditionalPanel(condition="input.showWP1",
				div(class="row-fluid",
					div(class="span12",selectInput("dataset", "Select data:", choices=dataset.names, selected=dataset.names[1], width="100%"))#,
		...
		wellPanel(
			...
			conditionalPanel(condition="input.showWP3",
				div(class="row-fluid",
					...
					div(class="span6", sliderInput("image.nlevels","Number of breaks:",6,30,16,2, width="100%"))
				),
				div(class="row-fluid",
					div(class="span6", selectInput("resfac", "Increase resolution by factor:", 1:4,1, width="100%")),
					div(class="span6",
				...
				div(class="row-fluid",
					...
					div(class="span6", selectInput("border", "Cell border color:", c("","White","Black"),"", width="100%"))
				),
				div(class="row-fluid",
					div(class="span6", selectInput("lighting", "Lighting:", c("None","Default"),"None", width="100%")),
					div(class="span6", sliderInput("shade","Shading:",0,1,0.5,0.05, width="100%"))
				),
				div(class="row-fluid",
					div(class="span6", 
						selectInput("image.asp", "Aspect ratio:", c("Original scale","Rescale to square"),"Original scale", width="100%")),
					div(class="span6", sliderInput("image.theta","Rotate (Uncheck smoothing):",-180,180,0,10, width="100%"))
				)
			)
		)
	),
				...
				div(class="row-fluid",
					div(class="span6", 
						selectInput("persp.col","Perspective colors:",
							c("R:Topo","R:Terrain","R:Heat","R:Cyan-Magenta","black-white","purple-yellow"),"R:Topo", width="100%")),
					div(class="span6", sliderInput("adjustrelief","Scale relief:",-10,10,0,2, width="100%"))
				),
				div(class="row-fluid",
					div(class="span6", 
						selectInput("persp.contourside","2D contour level(s):", c("","Below","Layered","Above"), "", multiple=T, width="100%")),
					div(class="span6", sliderInput("persp.zlim1","Decrease lower Z-limit:",0,5,0,0.5, width="100%"))
				),
				div(class="row-fluid",
					div(class="span6", selectInput("persp.imageside","2D image level:",c("","Below","Above"),"",multiple=T, width="100%")),
					div(class="span6", sliderInput("persp.zlim2","Increase upper Z-limit:",0,5,0,0.5, width="100%"))
				),
				div(class="row-fluid",
					div(class="span6", 
					...
					div(class="span6", sliderInput("theta","Azimuthal viewing angle:",-180,180,40,10, width="100%"))
				),
				div(class="row-fluid",
					div(class="span6", 
						selectInput("along", "Ribbons along:", c("x-axis","y-axis","both axes"),"x-axis", width="100%"), 
						checkboxInput("curtain","Drape edges")),
					div(class="span6", sliderInput("phi","Colatitudinal viewing angle:",-90,90,40,10, width="100%"))
				)
			)
		)
	),
